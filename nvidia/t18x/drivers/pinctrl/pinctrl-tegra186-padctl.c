...
static void tegra186_padctl_save(struct tegra_padctl *padctl)
{
	...
	padctl->padctl_context.vbus_oc_map =
...
static void tegra186_padctl_restore(struct tegra_padctl *padctl)
{
	...
	for (port = 0; port < TEGRA_UTMI_PHYS; port++) {
		if (padctl->utmi_ports[port].port_cap != HOST_ONLY)
			continue;

		/* only host port will be handled here */
		ocpin = padctl->utmi_ports[port].oc_pin;
		mutex_lock(&padctl->lock);
		if (padctl->oc_pinctrl && ocpin >= 0 &&
		!!(padctl->padctl_context.vbus_oc_map & VBUS_ENABLE(ocpin))) {
			rc = tegra_xusb_select_vbus_en_state(padctl,
				ocpin, true);
			if (rc == 0)
				tegra186_enable_vbus_oc(
					padctl->utmi_phys[port]);
		}
		mutex_unlock(&padctl->lock);
	}
