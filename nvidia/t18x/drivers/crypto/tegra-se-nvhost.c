...
struct tegra_se_req_context {
	...
	u8 hash_result[64]; /* Intermediate hash result */
...
#define TEGRA_SE_NO_SHA_PROCESS	1
...
static void tegra_se_write_hash_result(struct tegra_se_dev *se_dev, u32 *pdata,
				      u32 nbytes)
{
	u32 i;
	...
	for (i = 0; i < nbytes / 4; i++) {
		se_writel(se_dev, *(pdata + i), SE_HASH_RESULT_REG_OFFSET +
			     (i * sizeof(u32)));
	}
	nvhost_module_idle(se_dev->pdev);
}
				  ...
				  u32 count, bool last)
{
	...
	tegra_se_write_hash_result(se_dev, (u32 *)req_ctx->hash_result, 64);
	...
	while (total) {
		...
		if (total == count) {
			...
			msg_len = (count * 8);
			total_bits = ((u64)req_ctx->total_count)* 8ULL;
			cmdbuf_cpuvaddr[i++] = (u32)(total_bits &
							0xFFFFFFFFULL);
			cmdbuf_cpuvaddr[i++] = (u32)(total_bits >> 32);
			cmdbuf_cpuvaddr[i++] = 0;
			cmdbuf_cpuvaddr[i++] = 0;
...
static void tegra_se_read_hash_result(struct tegra_se_dev *se_dev, u8 *pdata,
				      u32 nbytes, bool swap32)
{
	u32 *result = (u32 *)pdata;
	u32 i;
...
static void tegra_se_sha_copy_residual_data(
		struct ahash_request *req, struct tegra_se_req_context *req_ctx,
		u32 bytes_to_copy)
{
				    ...
				    bool process_cur_req)
{
	...
	if (is_last) {
		...
		if (req_ctx->residual_bytes) {
			src_ll->addr = req_ctx->sha_buf_addr[0];
			src_ll->data_len = req_ctx->residual_bytes;
			src_ll++;
		}
		...
		if (num_blks <= 1) {
			...
			req_ctx->residual_bytes += req->nbytes;
			return TEGRA_SE_NO_SHA_PROCESS;
		}
		...
		src_ll = se_dev->src_ll;
		if (req_ctx->residual_bytes) {
			src_ll->addr = req_ctx->sha_buf_addr[1];
			src_ll->data_len = req_ctx->residual_bytes;
			src_ll++;
		}
		...
		req_ctx->residual_bytes = current_total - (num_blks *
							   req_ctx->blk_size);
		tegra_se_sha_copy_residual_data(req, req_ctx,
						req_ctx->residual_bytes);
	...
	err = tegra_se_send_sha_data(se_dev, req_ctx,
			   ...
			   bool process_cur_req)
{
	...
	case SHA1_DIGEST_SIZE:
		req_ctx->op_mode = SE_AES_OP_MODE_SHA1;
		break;
	case SHA224_DIGEST_SIZE:
		req_ctx->op_mode = SE_AES_OP_MODE_SHA224;
		break;
	case SHA256_DIGEST_SIZE:
		req_ctx->op_mode = SE_AES_OP_MODE_SHA256;
		break;
	case SHA384_DIGEST_SIZE:
		req_ctx->op_mode = SE_AES_OP_MODE_SHA384;
		break;
	case SHA512_DIGEST_SIZE:
		req_ctx->op_mode = SE_AES_OP_MODE_SHA512;
		break;
	default:
		return -EINVAL;
	}
	...
	if (!req->nbytes) {
		if (req_ctx->total_count) {
			 ...
			 */
				mode = req_ctx->op_mode - SE_AES_OP_MODE_SHA1;
	...
	/* If there is no HW processing, then
	 * do not read from HW
	 */
	if (ret == TEGRA_SE_NO_SHA_PROCESS)
		goto end;

	if (!is_last) {
		tegra_se_read_hash_result(se_dev, req_ctx->hash_result,
				64, false);
	}
	...
	if (is_last) {
		...
		if ((req_ctx->op_mode == SE_AES_OP_MODE_SHA384) ||
		    (req_ctx->op_mode == SE_AES_OP_MODE_SHA512)) {
...
static int tegra_se_sha_init(struct ahash_request *req)
{
	...
	req_ctx->blk_size = crypto_tfm_alg_blocksize(crypto_ahash_tfm(tfm));
...
static void tegra_se_sha_req_deinit(struct tegra_se_req_context *req_ctx)
{
	struct tegra_se_dev *se_dev = se_devices[SE_SHA];
	int i;
...
static int tegra_se_sha_update(struct ahash_request *req)
{
	...
	if (ret) {
		dev_err(se_dev->dev, "tegra_se_sha_update init failed - %d\n", ret);
		return -ret;
	}
...
static int tegra_se_sha_finup(struct ahash_request *req)
{
	...
	ret = tegra_se_sha_init(req);
	if (ret) {
		dev_err(se_dev->dev, "tegra_se_sha_finup init failed - %d\n", ret);
		return -ret;
	}
	...
	tegra_se_sha_req_deinit(req_ctx);
				    ...
				    unsigned int keylen)
{
	...
	ret = tegra_se_send_key_data(
		...
		se_dev->aes_cmdbuf_cpuvaddr, se_dev->aes_cmdbuf_iova, false);
	if (ret) {
		dev_err(se_dev->dev,
...
static struct ahash_alg hash_algs[] = {
	{
		...
		.halg.statesize = sizeof(struct tegra_se_req_context),
		.halg.base = {
			.cra_name = "sha1",
