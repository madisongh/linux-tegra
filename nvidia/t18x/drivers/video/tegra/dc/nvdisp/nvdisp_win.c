/*
 * drivers/video/tegra/dc/nvdisp/nvdisp_win.c
 *
...
int tegra_nvdisp_verify_win_downscaling_factor(struct tegra_dc *dc,
...
int tegra_nvdisp_verify_win_yuv_surface_is_even(struct tegra_dc *dc,
			struct tegra_dc_ext_flip_windowattr_v2 *win)
{
	fixed20_12 h_offset, v_offset, win_height, win_width;

	if (!tegra_dc_is_yuv(win->pixformat))
		return 0;
	...
	/* Check the cropped size width, height and surface offsets
	 * are even for YUV formats. Fail the call if not even
	 */
	if (NVDISP_ODD_VAL(dfixed_trunc(win_height))
		|| NVDISP_ODD_VAL(dfixed_trunc(win_width))
		|| NVDISP_ODD_VAL(dfixed_trunc(h_offset))
		|| NVDISP_ODD_VAL(dfixed_trunc(v_offset))) {
		dev_err(&dc->ndev->dev,
			"Win %d HEIGHT(%u),WIDTH(%u),X(%u),Y(%u) must be EVEN for YUV fmts(%u)\n",
			win->index,
			dfixed_trunc(win_height),
			dfixed_trunc(win_width),
			dfixed_trunc(h_offset),
			dfixed_trunc(v_offset),
			tegra_dc_fmt(win->pixformat));
		return -EINVAL;
	}
	return 0;
}
...
int tegra_nvdisp_verify_win_properties(struct tegra_dc *dc,
			struct tegra_dc_ext_flip_windowattr_v2 *win)
{
	if (tegra_nvdisp_verify_win_downscaling_factor(dc, win))
		return -EINVAL;

	if (tegra_nvdisp_verify_win_yuv_surface_is_even(dc, win))
		return -EINVAL;
...
static void tegra_nvdisp_win_attribute(struct tegra_dc_win *win,
