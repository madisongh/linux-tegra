...
static const u32 dsi_pkt_seq_video_non_burst_no_hbp[NUMOF_PKT_SEQ] = {
	PKT_ID0(CMD_VS) | PKT_LEN0(0) | PKT_ID1(CMD_EOT) | PKT_LEN1(7) | PKT_LP,
	0,
	PKT_ID0(CMD_HS) | PKT_LEN0(0) | PKT_ID1(CMD_EOT) | PKT_LEN1(7) | PKT_LP,
	0,
...
void tegra_dsi_init_clock_param(struct tegra_dc *dc)
{
	...
	/* Round up to multiple of kilo hz. */
	plld_clk_khz = DIV_ROUND_UP((byte_clk_hz * NUMOF_BIT_PER_BYTE), 1000);
	 ...
	 */
	dsi->default_hs_clk_khz = plld_clk_khz / 2;

	dsi->default_pixel_clk_khz = (plld_clk_khz *
	...
	if (dsi->info.pkt_seq)
		...
		default:
			if (dsi->info.ganged_type ||
				...
				pkt_seq =
				dsi_pkt_seq_video_non_burst_no_hbp;
			} else {
				pkt_seq_3_5_rgb_lo =
					DSI_PKT_SEQ_3_LO_PKT_32_ID(rgb_info);
