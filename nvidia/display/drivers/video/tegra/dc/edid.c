...
int tegra_edid_get_monspecs(struct tegra_edid *edid, struct fb_monspecs *specs)
{
	...
	/* For EDID struct v1.4, digital input only:
	 * byte 0x12 - EDID version number, this shall be 1 in this case
	 * byte 0x13 - EDID revision number, this shall be 4 in this case
	 * byte 0x14 - bit 7 input video signal interface:
			0: analog
			1: digital
	 * byte 0x14 - bit 6:4 color bit depth (if bit 7 is 1/digital):
			6: 16 bits (including 12-bit formats)
			4: 12 bits (including 10-bit formats)
			3: 10 bits
	 * byte 0x14 - bit 3:0 digital video interface supported
			1: DVI
			2: HDMI-a
			3: HDMI-b
			5: DisplayPort
	 * byte 0x18 - bit 4:3 supported color formats (if 0x14, bit 7 is 1)
			0: RGB 4:4:4
			1: RGB 4:4:4 & YCrCb 4:4:4
			2: RGB 4:4:4 & YCrCb 4:2:2
			3: RGB 4:4:4 & YCrCb 4:4:4 & YCrCb 4:2:2
	 */
	if (((data[0x12] == 0x01) && (data[0x13] == 0x04)) &&
			((data[0x14] & 0x80) == 0x80)) {
		if ((data[0x14] & 0x0f) == 0x05) {
			new_data->color_depth_flag = 0;
			switch ((data[0x14] & 0x70) >> 4) {
			/* 16 bits */
			case 6:
				new_data->color_depth_flag = TEGRA_DC_RGB_48;
				if (data[0x18] & 0x10)
					new_data->color_depth_flag |=
						TEGRA_DC_Y422_48;
				if (data[0x18] & 0x08)
					new_data->color_depth_flag |=
						TEGRA_DC_Y444_48;
			/* 12 bits */
			case 4:
				new_data->color_depth_flag = TEGRA_DC_RGB_36;
				if (data[0x18] & 0x10)
					new_data->color_depth_flag |=
						TEGRA_DC_Y422_36;
				if (data[0x18] & 0x08)
			...
			/* 10 bits */
			case 3:
				new_data->color_depth_flag = TEGRA_DC_RGB_30;
				if (data[0x18] & 0x10)
					new_data->color_depth_flag |=
						TEGRA_DC_Y422_30;
				if (data[0x18] & 0x08)
