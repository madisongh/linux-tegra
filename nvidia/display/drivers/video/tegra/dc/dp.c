...
static inline u32 tegra_dp_get_bpp(struct tegra_dc_dp_data *dp, u32 vmode)
{
	...
	if (yuv_flag == (FB_VMODE_Y422 | FB_VMODE_Y24)) {
		...
		dev_dbg(&dp->dc->ndev->dev, "%s: Unsupported mode with vmode: 0x%x for DP\n",
				__func__, vmode);
		return 0;
...
static inline u8 tegra_dp_vsc_get_colorimetry(struct tegra_dc_dp_data *dp)
{
	...
	if (vmode_flag & FB_VMODE_EC_ENABLE) {
		...
		case FB_VMODE_EC_BT2020_YCC_RGB:
			colorimetry = VSC_RGB_ITU_R_BT2020;
			break;
		default:
...
static void tegra_dp_vsc_col_ext(struct tegra_dc_dp_data *dp)
{
	...
	if (!cfg->support_vsc_ext_colorimetry)
		return;
					...
					struct clk *src)
{
	 ...
	 */
#ifdef CONFIG_TEGRA_NVDISPLAY
	tegra_disp_clk_disable_unprepare(sor->src_switch_clk);
	clk_set_parent(sor->src_switch_clk, src);
...
static void tegra_dc_dp_enable(struct tegra_dc *dc)
{
	...
	if (!dc->initialized) {
#ifdef CONFIG_TEGRA_NVDISPLAY
		tegra_dp_set_sor_clk_src(dp, sor->brick_clk);
#else
		tegra_sor_config_dp_clk(sor);
#endif
	}
