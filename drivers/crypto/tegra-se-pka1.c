...
static struct tegra_se_ecc_point *tegra_se_ecc_alloc_point(
				struct tegra_se_pka1_dev *se_dev, int nwords)
{
...
static void tegra_se_ecc_free_point(struct tegra_se_pka1_dev *se_dev,
				     ...
				     struct tegra_se_pka1_request *req)
{
	...
	case SE_PKA1_OP_MODE_RSA4096:
		for (i = 0; i < req->size / 4; i++) {
			*RES = se_pka1_readl(se_dev, reg_bank_offset(
	...
	case SE_PKA1_OP_MODE_ECC521:
		if (req->ecc_type == ECC_POINT_VER) {
			...
			for (i = 0; i < req->size / 4; i++) {
				*QX = se_pka1_readl(se_dev, reg_bank_offset(
						    TEGRA_SE_PKA1_ECC_XP_BANK,
						    TEGRA_SE_PKA1_ECC_XP_ID,
						    req->op_mode) + (i * 4));
				QX++;
			...
			for (i = 0; i < req->size / 4; i++) {
				*QY = se_pka1_readl(se_dev, reg_bank_offset(
						    TEGRA_SE_PKA1_ECC_YP_BANK,
						    TEGRA_SE_PKA1_ECC_YP_ID,
						    req->op_mode) + (i * 4));
				QY++;
...
static int tegra_se_ecc_point_mult(struct tegra_se_ecc_point *result,
				   ...
				   int nbytes)
{
	struct tegra_se_pka1_request ecc_req;
	int ret;
	...
	ret = tegra_se_pka1_op(&ecc_req);

	return ret;
}

static int tegra_se_ecdh_compute_shared_secret(struct tegra_se_pka1_dev *se_dev,
					       unsigned int cid,
...
static int tegra_se_ecdh_gen_pub_key(struct tegra_se_pka1_dev *se_dev,
