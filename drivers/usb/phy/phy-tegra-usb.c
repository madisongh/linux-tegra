...
static int tegra_usb_phy_get_clocks(struct tegra_usb_phy *phy)
{
	...
	if (tegra_platform_is_silicon()) {
		...
		phy->pllu_clk = devm_clk_get(&phy->pdev->dev, "pllu");
	...
	phy->ctrlr_clk = devm_clk_get(&phy->pdev->dev, "usbd");
...
int tegra_usb_phy_postsuspend(struct tegra_usb_phy *phy)
{
	int status = 0;
...
EXPORT_SYMBOL_GPL(tegra_usb_phy_postsuspend);

int tegra_usb_phy_preresume(struct tegra_usb_phy *phy, bool remote_wakeup)
{
	int status = 0;
...
struct tegra_usb_phy *tegra_usb_phy_open(struct platform_device *pdev)
{
	...
	if (phy->ops && phy->ops->open) {
		...
		if (err) {
			ERR("inst:[%d] Failed to open hw ops, err: %d\n",
				phy->inst, err);
			goto fail_init;
		}
	}
