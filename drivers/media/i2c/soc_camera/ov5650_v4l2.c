...
static int ov5650_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	ret = ov5650_write_table(priv, reset_seq);
	if (ret)
		return ret;

	ret = ov5650_write_table(priv, mode_start);
	if (ret)
		return ret;
	...
	ret = ov5650_write_table(priv, mode_end);
	if (ret)
		return ret;

	switch (test_mode) {
	case TP_NONE:
		ret = ov5650_write_table(priv, tp_none_seq);
		break;
	case TP_COLORBARS:
		ret = ov5650_write_table(priv, tp_cbars_seq);
		break;
	case TP_CHECKER:
		ret = ov5650_write_table(priv, tp_checker_seq);
		break;
	}
	if (ret)
		return ret;
			 ...
			 const struct i2c_device_id *did)
{
	...
	if ((priv->chip_id != 0x5650) && (priv->chip_id != 0x5651)) {
