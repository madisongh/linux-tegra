...
static struct camera_common_pdata *ov5693_parse_dt(struct i2c_client *client)
{
	...
	board_priv_pdata->use_cam_gpio = of_property_read_bool(np,
					 "cam,use-cam-gpio");
			...
			const struct i2c_device_id *id)
{
	...
	struct tegra_camera_platform_data *ov5693_camera_data;
	char node_name[10];
	int err;
	...
	ov5693_camera_data = (struct tegra_camera_platform_data *)
			     ssdd->drv_priv;
	if (!ov5693_camera_data) {
		dev_err(&client->dev, "unable to find iclink module name\n");
		return -EFAULT;
	}

	sprintf(node_name, "ov5693_%c", ov5693_camera_data->port + 'a');
	...
	common_data->csi_port		= (int)ov5693_camera_data->port;
	common_data->numlanes		= ov5693_camera_data->lanes;
	...
	v4l2_i2c_subdev_init(&common_data->subdev, client, &ov5693_subdev_ops);
